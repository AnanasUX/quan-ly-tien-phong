...
def goi_gemini_api(cau_hoi):
    # Danh sách model ưu tiên tốc độ – thứ tự fallback
    models = [
        "gemini-2.5-flash",
        "gemini-2.0-flash",
        "gemini-1.5-flash",
...
def tao_bao_cao_powerpoint():
    ...
    try:
        ...
        # 1. Lấy thư mục con của POWERPOINT_FOLDER_ID
        ...
        for main_folder in main_folders:
            ...
            # ── TRƯỜNG HỢP KHÔNG CÓ thu_muc_goc (quét trực tiếp) ──
            else:
                # Quét sâu 2 cấp thư mục con
...
def xac_dinh_trang_thai_thoi_tiet(c_temp, feels_like, c_desc, n_pop, humidity):
    ...
    la_mua_to       = la_mua_hien_tai and (
        any(x in desc_lower for x in ["to", "vừa", "nặng", "dông", "giông", "rào"]) or n_pop >= 80
    )
...
def lay_bang_diem_ngap(ten_vung, muc_do="NHE"):
    ...
    if muc_do == "NHE":
        ...
        for phuong, mn, mx, tuyen, hau_qua in ds:
            ...
            canh_bao = ("⛔ <b>CẤM XE GẦM THẤP</b>"     if max_v >= 50 else
                        "⚠️ <b>GẦM THẤP NGUY HIỂM</b>" if max_v >= 35 else
                        "⚠️ <b>ĐI CHẬM ĐỀU GA</b>")
...
def cap_nhat_trang_thai_thoi_tiet():
    ...
    try:
        ...
        so_ca_mua  = sum(1 for i in list_data if
                        i.get('pop', 0) >= 0.5 or 'mưa' in i['weather'][0]['description'].lower())
        so_ca_nang = sum(1 for i in list_data if
                        i['main']['temp_max'] >= 34 and i.get('pop', 0) < 0.2)
        if so_ca_mua >= 4:
            TRANG_THAI_THOI_TIET = "MUA_CA_NGAY"
        elif so_ca_nang >= 4:
            TRANG_THAI_THOI_TIET = "NANG_CA_NGAY"
        else:
            TRANG_THAI_THOI_TIET = "BINH_THUONG"
    except:
        pass
...
def tim_toa_do_theo_ten(ten_vung):
    try:
        url      = (f"http://api.openweathermap.org/geo/1.0/direct"
                    f"?q={urllib.parse.quote(ten_vung)}&limit=1&appid={OPENWEATHER_API_KEY}")
        res      = requests.get(url, timeout=10)
        res.encoding = 'utf-8'
        response = res.json()
        if response:
            kq       = response[0]
            loc_name = kq.get('local_names', {}).get('vi', kq['name'])
            return {"thanh_cong": True, "lat": kq["lat"], "lon": kq["lon"],
                    "name": f"{loc_name}, {kq.get('country', '')}"}
...
def lay_thoi_tiet_hom_nay_de_ghim():
    try:
        ...
        url_forecast = (f"https://api.openweathermap.org/data/2.5/forecast"
                        f"?lat={lat}&lon={lon}&appid={OPENWEATHER_API_KEY}&units=metric&lang=vi")
        res = requests.get(url_forecast, timeout=10)
        res.encoding = 'utf-8'
        res_json  = res.json()
        if str(res_json.get("cod")) != "200":
            return "⚠️ Lỗi máy chủ thời tiết."
        ...
        for item in list_data[:8]:
            ...
            line = (f"<code>{dt_val.strftime('%H:%M')}</code>: "
                    f"<code>{round(item['main']['temp'], 1)}°C</code> | "
                    f"Mưa <code>{int(item.get('pop', 0) * 100)}%</code> | "
                    f"{get_owm_icon(item['weather'][0]['icon'])} "
                    f"{item['weather'][0]['description'].capitalize()}")
            if 0 <= h < 13:
                frames_sang.append(line)
            else:
                frames_chieu.append(line)

        ht  = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=7)))
        msg = "🌤 <b>BẢN TIN THỜI TIẾT TỔNG QUAN HÔM NAY</b> 🌤\n"
        msg += "━━━━━━━━━━━━━━━━━━━━━━━\n"
        msg += f"📍 <b>Khu vực:</b> <code>{escape_html(ten_vi_tri)}</code>\n"
        msg += f"⏱ <b>Cập nhật:</b> <code>{ht.strftime('%H:%M - %d/%m/%Y')}</code>\n\n"
        msg += "📊 <b>TỔNG KẾT TRONG NGÀY</b>\n"
        ...
        msg += (f" └ Trạng thái: {get_owm_icon(list_data[0]['weather'][0]['icon'])} "
                f"<b>{list_data[0]['weather'][0]['description'].capitalize()}</b>\n\n")

        if frames_sang:
            msg += "🌅 <b>DIỄN BIẾN BUỔI SÁNG</b>\n"
            for i, line in enumerate(frames_sang):
                prefix = " └ " if i == len(frames_sang) - 1 else " ├ "
                msg += f"{prefix}{line}\n"
            msg += "\n"
        if frames_chieu:
            msg += "🌇 <b>DIỄN BIẾN BUỔI CHIỀU TỐI</b>\n"
        ...
        return f"⚠️ Lỗi dữ liệu thời tiết: {escape_html(str(e))}"
...
def lay_thoi_tiet_va_tin_tuc_hien_tai(include_news=True):
    try:
        ...
        msg += "🔮 <b>DỰ BÁO 3 GIỜ TỚI</b>\n"
        msg += f" └ ~<code>{n_temp}°C</code> | {n_desc} | Mưa: <code>{n_pop}%</code>\n\n"

        if trang_thai == "MUA_NHE":
            msg += "🚨 <b>CẢNH BÁO TRỌNG TÂM</b>\n"
            msg += " └ Mưa nhẹ đang diễn ra trên diện rộng, mặt đường có thể trơn trượt và tầm nhìn giảm.\n"
            msg += f" └ <b>Dự báo ngắn:</b> Xác suất mưa 3h tới vẫn ở mức <code>{n_pop}%</code>.\n\n"
            msg += "💡 <b>GỢI Ý LỊCH TRÌNH THỰC TẾ</b>\n"
            msg += " ├ <b>Điều khiển phương tiện:</b> Giảm tốc độ, giữ khoảng cách an toàn và hạn chế phanh gấp.\n"
            msg += " └ <b>Trang bị:</b> Mang áo mưa, bảo vệ điện thoại và các thiết bị điện tử khỏi nước.\n\n"
            msg += lay_bang_diem_ngap(ten_ngan, muc_do="NHE")
            img_url     = URL_ANH_NGAP
            img_caption = "🌦️ <b>CẢNH BÁO MƯA NHẸ</b>"
            tu_khoa     = f"mưa {ten_ngan}"

        elif trang_thai == "RONG_BAO":
            msg += "🚨 <b>CẢNH BÁO TRỌNG TÂM</b>\n"
            msg += (f" └ Mưa diện rộng (Độ ẩm <code>{humidity}%</code>). "
                    "Lượng mưa trút xuống nhanh đang gây quá tải hệ thống cống ngầm.\n")
            msg += f" └ <b>Dự báo ngắn:</b> Xác suất mưa 3h tới <code>{n_pop}%</code>, chưa có dấu hiệu tạnh hẳn.\n\n"
            msg += "💡 <b>GỢI Ý LỊCH TRÌNH THỰC TẾ</b>\n"
            msg += " ├ <b>Điều khiển phương tiện:</b> Không phanh gấp bằng phanh trước. Giảm tốc dưới 30km/h qua vũng đọng.\n"
            msg += " └ <b>Trang bị:</b> Dùng áo mưa bộ rời, bọc kỹ thiết bị điện tử.\n\n"
            msg += lay_bang_diem_ngap(ten_ngan, muc_do="TO")
            img_url     = URL_ANH_NGAP
            img_caption = "⛈️ <b>CẢNH BÁO DÔNG BÃO / MƯA TO</b>"
            tu_khoa     = f"ngập lụt OR mưa ngập {ten_ngan}"

        elif trang_thai == "SAP_MUA":
            msg += "🚨 <b>CẢNH BÁO TRỌNG TÂM</b>\n"
            msg += (f" └ Trời đang <b>tạnh ráo ({c_desc})</b>, nhưng mây dông đang tích tụ. "
                    f"Dự báo vài giờ tới có khả năng đổ mưa (Xác suất: <code>{n_pop}%</code>).\n\n")
            msg += "💡 <b>GỢI Ý LỊCH TRÌNH THỰC TẾ</b>\n"
            msg += " └ <b>Lộ trình:</b> Trời chưa mưa nhưng cần mang sẵn áo mưa dự phòng.\n\n"

        elif trang_thai == "NANG_GAT":
            msg += "🚨 <b>CẢNH BÁO TRỌNG TÂM</b>\n"
            msg += " └ 🔥 <b>Nắng nóng gay gắt:</b> Nhiệt độ cao, chỉ số UV ở mức nguy hiểm.\n"
            msg += f" └ <b>Dự báo ngắn:</b> Nhiệt độ tiếp tục duy trì mức cao trong 3h tới.\n\n"
            msg += "💡 <b>GỢI Ý LỊCH TRÌNH THỰC TẾ</b>\n"
            msg += " ├ <b>Điều khiển phương tiện:</b> Hạn chế di chuyển ngoài trời vào thời điểm nắng mạnh.\n"
            msg += " └ <b>Trang bị:</b> Sử dụng áo chống nắng, mũ, kính râm và kem chống nắng.\n\n"
            msg += lay_bang_nang_gat(ten_ngan, int(c_temp))
            img_url     = URL_ANH_NANG
            img_caption = "🔥☀️ <b>CẢNH BÁO NẮNG GẮT</b>"
            tu_khoa     = f"nắng nóng gay gắt {ten_ngan}"

        else:
            msg += "🚨 <b>CẢNH BÁO TRỌNG TÂM</b>\n"
            msg += " └ ☀️ <b>Thời tiết ổn định:</b> Trời quang, ít mây, khả năng mưa thấp.\n"
            msg += " └ <b>Dự báo ngắn:</b> Chưa ghi nhận nguy cơ mưa hay biến đổi đáng kể trong 3h tới.\n\n"
            msg += "💡 <b>GỢI Ý LỊCH TRÌNH THỰC TẾ</b>\n"
            msg += " ├ <b>Điều khiển phương tiện:</b> Điều kiện đường sá thuận lợi, di chuyển bình thường.\n"
            msg += " └ <b>Trang bị:</b> Có thể mang kính râm, mũ và nước uống nếu hoạt động ngoài trời lâu.\n\n"
        ...
        if cmd in ["/start", "/menu", "Menu", "/help"]:
            ...
            if is_admin:
                ...
                txt_menu = ("<b>🤖 HỆ THỐNG ANX - QUẢN LÝ TIỀN PHÒNG & THÔNG TIN</b>\n"
                            ...
                            "• <code>/tinh</code> : Nhập số điện nước & tính hóa đơn\n"
...
@app.route('/')
def home():
    return "Hệ thống AnX v7.0 (Token mới 09/2026 – Gemini 2.5 Flash – thu_muc_goc) đang hoạt động!"
